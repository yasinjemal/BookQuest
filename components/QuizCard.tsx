...
export default function QuizCard({
  ...
  const initialChoice = typeof answerResult?.answer === "number" || typeof answerResult?.answer === "boolean" ? answerResult.answer : null;
  const [choice, setChoice] = useState<number | boolean | null>(initialChoice);
  const [draftChoice, setDraftChoice] = useState<number | boolean | null>(initialChoice);
  const [typed, setTyped] = useState(typeof answerResult?.answer === "string" ? answerResult.answer : "");
  const [revealed, setRevealed] = useState(Boolean(answerResult));
  const [correct, setCorrect] = useState(Boolean(answerResult?.correct));

  const submit = useCallback((answer: number | boolean | string) => {
  ...
  }, [card, onAnswered, revealed, startedAt]);

  useEffect(() => {
    if (!answerResult) return;
    submitted.current = true;
    if (typeof answerResult.answer === "number" || typeof answerResult.answer === "boolean") {
      setChoice(answerResult.answer);
      setDraftChoice(answerResult.answer);
    } else if (typeof answerResult.answer === "string") {
      setTyped(answerResult.answer);
    }
    setCorrect(answerResult.correct);
    setRevealed(true);
  }, [answerResult]);
  ...
  useEffect(() => {
    if (!immersive || revealed || card.type === "quiz_fillblank") return;
  ...
  }, [card, draftChoice, immersive, revealed, submit]);
  ...
  const choose = (answer: number | boolean) => {
    if (revealed) return;
    if (immersive) setDraftChoice(answer);
    else submit(answer);
  };
  ...
  return (
    <article className={`lesson-quiz overflow-hidden border border-dusk/25 bg-card ${immersive ? "lesson-quiz-interlude" : ""}`} data-revealed={revealed} data-correct={revealed ? correct : undefined}>
      <header className={`lesson-quiz-header border-b border-line bg-dusk text-white ${immersive ? "px-6 py-7 sm:px-9 sm:py-9" : "px-5 py-5 sm:px-6"}`}>
        <div className="lesson-quiz-label flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-white/75"><AppIcon name={immersive ? "spark" : "compass"} className="h-4 w-4" />{immersive ? "Retrieval challenge" : "Decision moment"}</div>
        ...
        {card.type === "quiz_fillblank" && <h2 className={questionClass}>Complete the thought</h2>}
      </header>
      ...
      <div className={`lesson-quiz-body text-ink ${immersive ? "p-6 sm:p-9" : "p-5 sm:p-6"}`}>
        ...
        {immersive && !revealed && card.type !== "quiz_fillblank" && <div className="mt-5 flex flex-col items-center justify-between gap-3 rounded-2xl border border-line bg-paper/60 p-3 sm:flex-row"><p className="px-2 text-xs leading-5 text-ink-soft">Choose first. Lock it only when the answer feels right.</p><button type="button" onClick={() => { if (draftChoice !== null) submit(draftChoice); }} disabled={draftChoice === null} className="btn-primary min-w-40"><AppIcon name="lock" className="h-4 w-4" />Lock answer</button></div>}

        {card.type === "quiz_fillblank" && <><p className={`reading rounded-2xl border border-line bg-ivory ${immersive ? "p-6 text-lg sm:p-8" : "p-5"}`}>{card.sentence.split("___").map((part, index, parts) => <span key={index}>{part}{index < parts.length - 1 && <span className="mx-1 inline-block min-w-24 border-b-2 border-teal px-1 text-center font-semibold text-teal-deep">{revealed ? card.answer : typed || "\u00a0"}</span>}</span>)}</p>{!revealed && <form onSubmit={(event) => { event.preventDefault(); if (typed.trim()) submit(typed); }} className="mt-5 flex flex-col gap-3 sm:flex-row"><label htmlFor="fill-answer" className="screen-reader-text">Your answer</label><input id="fill-answer" value={typed} onChange={(event) => setTyped(event.target.value)} placeholder="Type what you remember" className="field min-w-0 flex-1" /><button type="submit" disabled={!typed.trim()} className="btn-primary sm:shrink-0">Check answer</button></form>}</>}

        {revealed && <div ref={feedbackRef} tabIndex={-1} id={feedbackId} role="status" className={`quiz-feedback mt-5 rounded-2xl border px-5 py-5 ${correct ? "course-feedback-success" : "course-feedback-error"}`} data-result={correct ? "correct" : "incorrect"}><div className="flex items-center gap-2 font-semibold"><AppIcon name={correct ? "check" : "compass"} className="h-4 w-4" />{correct ? "Path opened." : "Correction collected."}</div><p className="mt-2 text-sm leading-6 opacity-80">{card.explanation}</p>{immersive && <p className="mt-3 text-xs font-semibold uppercase tracking-[0.12em] opacity-65">{correct ? "You recalled it without the source in view." : "Carry the corrected idea into the next moment."}</p>}</div>}
      </div>
    </article>
  );
}
