...
export default function InstitutionalPilotPage() {
  ...
  return <div className="page-wrap mx-auto max-w-4xl space-y-5">
    ...
    {active && canObserve && <form onSubmit={observe} className="rounded-2xl border border-line bg-card p-4 space-y-3">
      ...
      <div className="grid gap-2 sm:grid-cols-2"><select aria-label="Observation type" value={observationType} onChange={(event) => setObservationType(event.target.value)} className="rounded-xl border border-line bg-paper p-2.5 text-sm"><option value="admin_journey">Administrator journey</option><option value="learner_journey">Learner journey</option><option value="support">Support need</option><option value="commercial">Commercial discussion</option><option value="incident">Incident exercise</option></select><input aria-label="Participant code" value={participantKey} onChange={(event) => setParticipantKey(event.target.value)} placeholder="Opaque code, e.g. admin-01" className="rounded-xl border-2 border-line bg-paper px-3 py-2 text-sm" /></div>
      <textarea aria-label="Observation summary" value={observationSummary} onChange={(event) => setObservationSummary(event.target.value)} placeholder="What was observed?" rows={3} className="w-full rounded-xl border-2 border-line bg-paper px-3 py-2 text-sm" />
      <textarea aria-label="Support needs" value={supportNeeds} onChange={(event) => setSupportNeeds(event.target.value)} placeholder="Support needs, one per line" rows={2} className="w-full rounded-xl border border-line bg-paper px-3 py-2 text-sm" />
    ...
    {active && canManage && <form onSubmit={attest} className="rounded-2xl border border-line bg-card p-4 space-y-3">
      ...
      <div className="grid gap-2 sm:grid-cols-2"><select aria-label="Gate type" value={gateType} onChange={(event) => setGateType(event.target.value as GateType)} className="rounded-xl border border-line bg-paper p-2.5 text-sm">{GATES.map((gate) => <option key={gate.value} value={gate.value}>{gate.label}</option>)}</select><select aria-label="Gate outcome" value={outcome} onChange={(event) => setOutcome(event.target.value)} className="rounded-xl border border-line bg-paper p-2.5 text-sm"><option value="accepted">Accepted</option><option value="accepted_with_actions">Accepted with transparent actions</option><option value="rejected">Rejected</option></select></div>
      <textarea aria-label="Gate decision summary" value={gateSummary} onChange={(event) => setGateSummary(event.target.value)} placeholder="Decision, responsible stakeholder and stated purpose" rows={3} className="w-full rounded-xl border-2 border-line bg-paper px-3 py-2 text-sm" />
      {gateType === "audit_pack_acceptance" && <select aria-label="Accepted audit pack" value={auditPackId} onChange={(event) => setAuditPackId(event.target.value)} className="w-full rounded-xl border border-line bg-paper p-2.5 text-sm"><option value="">Select the accepted audit pack</option>{dashboard.evidenceCandidates?.auditPacks.map((pack) => <option key={pack.id} value={pack.id}>{pack.report_format_version} · {new Date(pack.created_at).toLocaleString()}</option>)}</select>}
      {gateType === "live_credential_revocation" && <select aria-label="Tested credential" value={credentialId} onChange={(event) => setCredentialId(event.target.value)} className="w-full rounded-xl border border-line bg-paper p-2.5 text-sm"><option value="">Select the tested credential</option>{dashboard.evidenceCandidates?.credentials.map((credential) => <option key={credential.id} value={credential.id}>{credential.display_code} · {credential.status}</option>)}</select>}
      <div className="grid gap-2 sm:grid-cols-2"><input aria-label="Evidence link" value={evidenceUri} onChange={(event) => setEvidenceUri(event.target.value)} placeholder="HTTPS evidence link" className="rounded-xl border border-line bg-paper px-3 py-2 text-sm" /><input aria-label="Artifact SHA-256 hash" value={artifactHash} onChange={(event) => setArtifactHash(event.target.value)} placeholder="SHA-256 artifact hash" className="rounded-xl border border-line bg-paper px-3 py-2 text-sm" /></div>
      {outcome === "accepted_with_actions" && <textarea aria-label="Open remediation actions" value={openActions} onChange={(event) => setOpenActions(event.target.value)} placeholder="Open remediation actions, one per line" rows={2} className="w-full rounded-xl border border-line bg-paper px-3 py-2 text-sm" />}
