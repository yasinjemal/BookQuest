...
function ContinueJourney({ course }: { course: CourseSummary }) {
  ...
  return (
    ...
    <section aria-labelledby="continue-journey-heading" className="grid overflow-hidden rounded-[1.75rem] bg-pine text-white shadow-pop lg:grid-cols-[.9fr_1.1fr]">
      <div className="relative min-h-44 overflow-hidden sm:min-h-56 lg:min-h-full">
        <CourseWorld seed={course.id} title={title} theme={appearance.worldTheme} accent={COURSE_ACCENT_HEX[appearance.accent]} progress={progress} mood={appearance.atmosphere === "full" ? "bright" : "calm"} className="absolute inset-0 min-h-full" />
        <ArtifactCoverImage kind="course" artifactId={course.id} contentHash={course.coverHash} variant="course" priority />
      </div>
