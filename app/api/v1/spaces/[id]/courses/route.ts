...
import { authenticateApiRequest, API_VERSION, listApiCourses } from "@/lib/integrations";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const auth = await authenticateApiRequest(req.headers.get("authorization"), spaceId, "courses.read");
    ...
    return NextResponse.json({ apiVersion: API_VERSION, data: await listApiCourses(spaceId) }, { headers: integrationPrivateHeaders });
  } catch (error) {
